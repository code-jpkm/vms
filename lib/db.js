...
export async function updateVendorStatus({ vendorId, status, adminNotes, rejectionReason, adminUserId = null }) {
  return withClient(async (client) => {
    await client.query('BEGIN');
    try {
      // 1) Update vendor
      ...
      // 2) Update latest application for that vendor
      const appRes = await client.query(
        ...
        SET status = $1::text,
            ...
            approval_date = CASE WHEN $1::text = 'approved' THEN CURRENT_TIMESTAMP ELSE approval_date END,
            updated_at = CURRENT_TIMESTAMP
        WHERE vendor_id = $4::int
          AND id = (
            SELECT id
            FROM applications
            WHERE vendor_id = $4::int
        ...
        RETURNING *
        `,
        [status, adminNotes, rejectionReason, vendorId]
      );

      // 3) Audit log (details is JSONB → cast explicitly)
      const detailsObj = { status, adminNotes, rejectionReason };

      await client.query(
        `
        INSERT INTO audit_logs (vendor_id, user_id, action, details)
        VALUES ($1::int, $2::int, $3::text, $4::jsonb)
        `,
        [
          vendorId,
